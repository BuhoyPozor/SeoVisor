...
function getRowHeadersImages() {
	...
	output += "<tbody>";
	output += "  <tr>";
	if (numberH1 == 0 || numberH1 > 1) {output += "    <td id='seoH1' class='error-value'>" + numberH1 + "</td>";} else {output += "    <td id='seoH1'>" + numberH1 + "</td>";}
	if (numberH2 == 0 || numberH2 > 2) {output += "    <td id='seoH2' class='error-value'>" + numberH2 + "</td>";} else {output += "    <td id='seoH2'>" + numberH2 + "</td>";}
	if (numberH3 > 3) {output += "    <td id='seoH2' class='error-value'>" + numberH3 + "</td>";} else {output += "    <td id='seoH3'>" + numberH3 + "</td>";}
...
function getHeadersDetail() {
	...
	output = "<span class='intro-tab'>Все заголовки в порядке их появления в HTML.</span>" +
		"<hr/>";
	...
	headersFound = false;
	$('h1,H1,h2,H2,h3,H3,h4,H4,h5,H5,h6,H6,strong,STRONG,b,B').each(function () {
...
function initBasicTab() {
	...
	if (msgLength == '0') {
		...
		if (isLengthError(msgLength, 30, 65)) {
			msgLength = "<span class='error-value'>" + msgLength + " Символов</span>";
		} else {
			msgLength = "<span class='good-value'>" + msgLength + " Символов</span>";
		}
	}
	...
	msgLengthError = "";
	if (msgLength == '0') {
		content = "<span class='error-message'>Нету Description!</span>";
		msgLength = "";
	} else {
		if (isLengthError(msgLength, 120, 156)) {
			msgLength = "<span class='error-value'>" + msgLength + " Символов</span>";
			msgLengthError = "It is a 120-156 characters page description"
		} else {
			msgLength = "<span class='good-value'>" + msgLength + " Символов</span>";
			msgLengthError = "It is a 120-156 characters page description"
		}
	...
	msgLengthError = "";
	if (msgLength == '0') {
		content = "<span class='no-data'>Нету Keywords!</span>";
		msgLength = "";
	} else {
		...
		if (content == '' || content == null || content.trim().length == 0) {
			...
			if (isLengthError(msgLength, 1, 200)) {
				msgLength = "<span class='error-value'>" + msgLength + " Символов</span>";
				msgLengthError = "It is a 120-156 characters page description"
			} else {
				msgLength = "<span class='good-value'>" + msgLength + " Символов</span>";
	...
	if (getLengthOf(content) == '0') {
		content = getMsgForNoData("Канонический URL");
	}
	output += getTitleAndContentOnOneLines(
		"Canonical",
		"",
...
function initLinksTab() {
	...
	tempOutput += "<div class='col-xs-3 bulle-info'><span class='sub-info'>without TITLE</span><br/><span class='main-info-sub " + className + "'>" + (numberOfLinks - numberOfTitle) + "</span></div>";
	tempOutput += "</div><hr/>";
...
function initToolsTabHtml() {
	...
	linkDesc = "Проверка WHOIS";
	linkImage = "https://www.google.com/s2/favicons?domain=http://2whois.ru/";
	...
	*/
	...
	link = "https://dev.twitter.com/docs/cards/validation/validator"
	linkName = "Twitter Validator";
	linkDesc = "Twitter Validator.";
